...
def experiment_combination(domain, seed):
    ...
    for d in domain:  # make sure the d doesn't overwrite
        # write every 5 seeds into one file
        for s in seed:
            command = "python3 discovhar_DANN.py --dataset dsads --model dsads64 --batch_size 256 --learning_rate 0.08 --temp 0.07 --epochs 2 --classifier_epochs 2 --cuda_device 1 --LODO %s --weighted_supcon 1 --seed %s" % (
                d, s)
