...
def experiment_combination(domain, seed, loro_loao):
    ...
    for d in domain:
        ...
        for r in loro_loao:
            for s in seed:
                command = "python3 discovhar_DANN.py --dataset dsads --model dsads64 --batch_size 256 --learning_rate 0.08 --temp 0.07 --epochs 10 --classifier_epochs 10 --LODO %s --weighted_supcon 1 --seed %s --loro %s --loao %s" % (
                    d, s, r[0], r[1])
                output_file = os.path.join(
                    output_folder, f"output_loro_{r[0]}_lodo_{d}_loao_{r[1]}.txt")
...
if __name__ == '__main__':
    ...
    experiment_combination(domain, seed, loro_loao)
